    ...
    bcrypt.hash(sntzedPassword, 10, (err, hash) => {   // 해싱
        if(err) throw err;
          ...
          db.query(`INSERT INTO user (loginid, password, name, mf, address, tel, birth, class) VALUES (?, ?, ?, ?, ?,?, ?, ?)`,
            [sntzedLoginid, hash, sntzedName,sntzedMf,sntzedAddress,sntzedtel,sntzedbirth,'CST'],(err,result)=>{
               if (err.code === 'ER_DUP_ENTRY') {
                  return res.send(`<script>alert('이미 존재하는 아이디입니다. 다른 아이디를 사용해주세요.'); 
                  location.href='/register/register';</script>`); }
                  
              else{  return res.send(`<script>alert('회원가입 성공!'); 
            location.href='/register/login';</script>`);
              }
            })
                })
            },
